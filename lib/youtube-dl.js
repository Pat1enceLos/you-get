...
function processData (data, options, stream) {
  ...
  console.log(url.parse(item.streams['flv'].src[0]));
  const headers = { Host: url.parse(item.streams['flv'].src[0]).hostname };
  console.log(item);
  headers['Referer'] = item.extra.referer;
  headers['Accept-Encoding'] = "gzip, deflate";
  headers['User-Agent'] = item.extra.ua;
  ...
  req.on('response', function response (res) {
    const size = parseInt(res.headers['content-length'], 10) ? parseInt(res.headers['content-length'], 10)
      : item.streams[Object.keys(item.streams)[0]].size;
    const format = item.streams[Object.keys(item.streams)[0]].container;
